...
class CoreMariaDBConnection(CoreDBConnection):
    """
        ...
        query = """
                SELECT sl.site_name,
                       ...
                       CASE
                           ...
                           WHEN s.last_updated >= ? THEN 'live_current'
                           WHEN s.last_updated >= ? THEN 'live_1_behind'
                           WHEN s.last_updated >= ? THEN 'live_l24_behind'
                           ELSE 'live_inactive'
    ...
    def get_recent_logs(self, log_level: str = None, site_id: str = None) -> list:
        ...
        if log_level:
            query += " AND UPPER(log_level) = UPPER(?)"
            params.append(log_level)

        if site_id:
            query += " AND UPPER(site_id) = UPPER(?)"
                ...
                with conn.cursor() as cursor:
                    ...
                    for row in results:
                        dict_results.append({
                            ...
                            'log_level': row[3],
                            'timestamp': row[4],
                            'summary_message': row[5],
                            'detailed_message': row[6]
                        })
    ...
    def get_log_count_last_24h(self, log_level: str) -> int:
        ...
        try:
            ...
            self.logger.error(f"MariaDB error counting log records: {e}")
            return 0
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    if drop_existing:
                        delete_query = "DELETE FROM remotes_hash_status WHERE site_name = ?"
