...
class RestAPITests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.hash_info_key = '/squishy/tests'
        ...
        self.REST_API = 'http://restapi:5000'

    def test_01_environment(self):
        # Tests to ensure that the container environment has the
        # required variables to run the rest api package
        print("Test that all required container environment variables are set\n")
        env_complete = True
        for variable in ['LOCAL_MYSQL_NAME',
            ...
            if not environ.get(variable):
                print(f"ERROR: To bootstrap rest_api {variable} env variable must exist")
                env_complete = False

        self.assertTrue(env_complete, "Container environment variables not set\n")

    def test_put_hashes(self):
        code, update = self._db_put("hashtable",
                               {'path': self.hash_info_key,
                                'current_hash': self.test_hash_info[self.hash_info_key]['current_hash'],
                                'current_dtg_latest': self.test_hash_info[self.hash_info_key]['current_dtg_latest'],
                                'dirs': self.test_hash_info[self.hash_info_key].get('dirs'),
                                'files': self.test_hash_info[self.hash_info_key].get('files'),
                                'links': self.test_hash_info[self.hash_info_key].get('links')
                                })
        print(code)
        print(update)
    ...
    #
    def _db_put(self, endpoint, json_request):
        return _db_contact('put', f'{self.REST_API}/{endpoint}', json_request)

    def _db_get(self, endpoint, params=None):
        return _db_contact('get', f'{self.REST_API}/{endpoint}', params)
